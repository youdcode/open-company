...
}
{
  ...
  ok('the AI gets read access to that folder and is told to only read it', withDir.text.includes(`can read: ${refDir}`) && (await (await fetch(`${base}/api/chat`)).json()).readDirs[0] === refDir);
  await setDirs([]);
